...
##can transform into a dubstep remix as "WUBWUBIWUBAMWUBWUBX" and
##cannot transform into "WUBWUBIAMWUBX".
##
##Recently, Jonny has heard Polycarpus's
##new dubstep track, but since he isn't into modern music,
...
def song_decoder(song):
    ...
    #remove ending spaces, strip and join with a space to replace one or
    #more spaces between words with only one space
    return ' '.join((fstr.strip(' ')).split())

#for example: this would print 'WE ARE THE CHAMPIONS MY FRIEND'
print so("WUBWEWUBAREWUBWUBTHEWUBCHAMPIONSWUBMYWUBFRIENDWUB")
